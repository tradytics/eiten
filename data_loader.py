...
class DataEngine:
	...
	def get_data(self, symbol):
		...
		if self.DATA_GRANULARITY_MINUTES == 3600:
			period = "5y"
			interval = "1d"
		else:
			period = "30d"
		...
		try:
			...
			if len(self.stock_data_length) > 5:
				...
				if data_length != most_frequent_key and data_length != self.HISTORY_TO_USE and symbol != self.MARKET_INDEX: # Need SPY at any cost
					return [], [], True

			if self.IS_TEST == 1:
				stock_prices_list = stock_prices.values.tolist()
				if type(self.HISTORY_TO_USE) == str and self.HISTORY_TO_USE == "all":
					stock_prices_list = stock_prices_list[1:]  # For some reason, yfinance gives some 0 values in the first index
				else:
					stock_prices_list = stock_prices_list[-self.HISTORY_TO_USE:]
	...
	def collect_data_for_all_tickers(self):
		...
		historical_price_info, future_price_info, symbol_names = self.remove_bad_data(historical_price_info, future_price_info, symbol_names)
		for i in range(0, len(symbol_names)):
			self.data_dictionary[symbol_names[i]] = {"historical_prices": historical_price_info[i], "future_prices": future_price_info[i]}
	...
	def remove_bad_data(self, historical_price_info, future_price_info, symbol_names):
		...
		length_dictionary = collections.Counter([len(item.values.tolist()) for item in historical_price_info])
