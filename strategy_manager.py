...
class StrategyManager:
    ...
    def calculate_genetic_algo_portfolio(self, symbols, returns_matrix_percentages):
        ...
        portfolio_weights_dictionary = self.geneticAlgoStrategy.generate_portfolio(
            symbols, returns_matrix_percentages)
        return portfolio_weights_dictionary

    def calculate_eigen_portfolio(self, cov_matrix, eigen_portfolio_number):
        """
        2nd Eigen Portfolio
        """
        print("-$ Calculating portfolio weights using eigen values...")
        portfolio_weights_dictionary = self.eigenPortfolioStrategy.generate_portfolio(
            cov_matrix, eigen_portfolio_number)
        return portfolio_weights_dictionary
    ...
    def calculate_minimum_variance_portfolio(self, symbols, covariance_matrix):
        ...
        portfolio_weights_dictionary = self.minimumVarianceStrategy.generate_portfolio(
            symbols, covariance_matrix)
        return portfolio_weights_dictionary

    def calculate_maximum_sharpe_portfolio(self, cov_matrix, returns_vector):
        """
